...
## 七、JWT 主動失效：登出與 Token 撤銷 <!-- 💡 進階 -->

JWT 的最大問題是**無狀態**——伺服器不存 Token，所以無法單方面讓一個 Token 失效（在過期前）。這讓「登出」成為一個棘手問題。

### 問題場景

```
用戶登出 → 前端刪除 Token → 但如果有人事先複製了這個 Token，
在過期前（可能還有 23 小時）仍然可以用它存取 API
```
...
public class TokenBlacklistService {
    ...
    // 登出時：把 Token 加入黑名單，TTL 設為 Token 剩餘有效期
    public void blacklist(String token) {
        Claims claims = jwtUtil.parseClaims(token);
        long remainingMs = claims.getExpiration().getTime() - System.currentTimeMillis();
        if (remainingMs > 0) {
            // key = "blacklist:{token}", value = "1", TTL = Token 剩餘時間
...
**缺點：** 每個請求都要查 Redis（但 Redis 速度極快，~0.1ms，可接受）。Token 過期後 Redis 自動清除 key，不會無限積累。

### 解法二：縮短 Token 有效期 + Refresh Token

```
Access Token：有效期 15 分鐘（短，洩露影響小）
Refresh Token：有效期 7 天（長，存在 DB 可主動撤銷）

流程：
  登入 → 發兩個 Token
  每次請求 → 帶 Access Token
  Access Token 快過期 → 前端用 Refresh Token 換新 Access Token
  登出 → 刪除 DB 裡的 Refresh Token（下次 refresh 就失效）
```
...
public class RefreshToken {
    ...
    private boolean revoked;            // 登出時設為 true
}
...
public TokenResponse refresh(@RequestBody String refreshToken) {
    ...
    // 發新的 Access Token
    String newAccessToken = jwtUtil.generate(rt.getUserId());
...
| | Redis 黑名單 | Refresh Token |
|---|---|---|
| 實作複雜度 | 低 | 高 |
| 即時失效 | 是（立即） | 否（要等 Access Token 過期）|
| 基礎設施依賴 | 需要 Redis | 需要 DB 表 |
| 適合場景 | 安全要求高的系統 | 一般 Web/App |
